...
// Map Viewport settings
    ...
    const expandAssist = () => {
        ...
        console.log("triggered assist ", offsetLeft, offsetTop)
        setAssistivePos({x: 0, y: 0})
        div.style.transition = `left 0.5s, top 0.5s, cubic-bezier(0.64, 0.57, 0.67, 1.53), height 0.5s 0.2s, width 0.5s 0.2s`;
    ...
    const collapseAssist = () => {
        console.log("triggered collapse and pos remembered is ", assistivePos)
        let div = document.getElementById("assistive-touch-div");
        const offsetLeft = assistivePos.x;
        const offsetTop = assistivePos.y;
        div.style.transition = `transform 0.2s cubic-bezier(0.64, 0.57, 0.67, 1.53), height 0.5s 0.2s, width 0.5s 0.2s`;
        div.style.transform = `translate(${offsetLeft}, ${offsetTop})`;
    }
