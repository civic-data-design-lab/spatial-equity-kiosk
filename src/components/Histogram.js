...
const Histogram = ({
    ...
    useEffect(() => {
        ...
        svg
            ...
            .text(
                !ascending
                    ? `${hiStatement} ${getIssueStatement()} ${d3.max(data) >= 10
                        ? d3.max(data).toFixed(0)
                        : d3.max(data) >= 1
                            ? d3.max(data).toFixed(2)
                            : d3.max(data)
                    }${metricSymbol}`
                    : `${lowStatement} ${getIssueStatement()} ${d3.min(data) >= 10
        ...
        svg
            ...
            .on('mousemove', function (event, d) {
                ...
                svg
                    ...
                    .attr('lookupID', lookupArray[rectID])
                    .text(
                        `${boundary == 'council' ? 'Council' : ''} ${nameArray[rectID]}${boundary == 'council'
        ...
        svg
            ...
            .text(
                (d, i) =>
                    `${boundary == 'council' ? 'Council' : ''} ${nameArray[i]}${boundary == 'council'
