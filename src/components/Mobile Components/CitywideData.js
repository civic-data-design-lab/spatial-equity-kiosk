...
import {faCaretDown, faCaretUp, faChevronRight, faMinus, faPlus} from "@fortawesome/free-solid-svg-icons";
...
export default function CitywideData({
                                         ...
                                         councilPinned,
                                         setCouncilPinned,
                                         info, setToggleUnderperformers, binList
    ...
    const getSelectionIssues = (category, cat_id) => {
        return (
            <div
                style={{
                    maxHeight: selectedIssue === cat_id && !selectedSpecificIssue ? "50%" : "0",
                    transition: "height 0.5s, opacity 0.5s",
                    pointerEvents: selectedIssue === cat_id && !selectedSpecificIssue ? "" : "none",
                    opacity: selectedIssue === cat_id && !selectedSpecificIssue ? "1" : "0",
                    overflow: "auto",
                    border: selectedIssue === cat_id && !selectedSpecificIssue ? "1px solid black" : "none"
            ...
            >
                <Table
                    className={`mobile-issue-dropdown ${selectedIssue === cat_id ? "mobile-issue-dropdown-grow" : "mobile-issue-dropdown-shrink"}`}>
                    <tbody>

                    {issues.issues_data[category].specific_issues_ID.map((id_) => {
                        return <tr
                            className={`${selectedSpecificIssue === id_ ? "active-scheme" : "inactive-scheme"} transition-color`}
                            onClick={(e) => {
                        ...
                        >
                            <td className={`${selectedIssue === cat_id && !selectedSpecificIssue ? "small-text opacity-100" : "no-text no-padding opacity-0"} mobile-issue-dropdown-item`}>
    ...
    return (
        <div className={"mobile-citywide"}>
            ...
            ${!selectedSpecificIssue ? "big-padding regular-border" : "no-border"}
            row-gap
            `}
                 style={{height: (selectedIssue && selectedIssue !== 1) && !selectedSpecificIssue ? "calc(1.375rem + 1.5vw + 3rem)" : selectedSpecificIssue ? "0vh" : !selectedIssue ? "calc((100vh - 4.025rem - 0.3vw) / 3)" : "calc((100vh - 4.025rem - 0.3vw) - 2 * (1.375rem + 1.5vw + 3rem)"}}
                 onClick={() => {
                     if (selectedIssue !== 1) {
                         setSelectedIssue(1)
                     } else {
            ...
            >
                <div className={`d-flex flex-row align-items-center justify-content-between`}>
                    <p className={`mb-0 ${!selectedSpecificIssue ? "big-text" : "no-text"} mobile-transition-font`}>
                        Health
                    </p>
                </div>
                <p className={`mb-0 mobile-transition-font
                ${(selectedIssue === 1 || !selectedIssue) && !selectedSpecificIssue ? "small-text" : "no-text"}`}>
            ...
            <div className={`mobile-citywide-chapter
            ${!selectedSpecificIssue ? "big-padding regular-border" : "border-none"}
                       row-gap`}
                 style={{height: (selectedIssue && selectedIssue !== 2) && !selectedSpecificIssue ? "calc(1.375rem + 1.5vw + 3rem)" : selectedSpecificIssue ? "0vh" : !selectedIssue ? "calc((100vh - 4.025rem - 0.3vw) / 3)" : "calc((100vh - 4.025rem - 0.3vw) - 2 * (1.375rem + 1.5vw + 3rem)"}}
                 onClick={() => {
                     if (selectedIssue !== 2) {
                         setSelectedIssue(2)
                     } else {
            ...
            >
                ...
                <p className={`mb-0 mobile-transition-font
                ${(selectedIssue === 2 || !selectedIssue) && !selectedSpecificIssue ? "small-text" : "no-text"}`}>
            ...
            ${!selectedSpecificIssue ? "big-padding regular-border" : "border-none"}
             row-gap`}
                 style={{height: (selectedIssue && selectedIssue !== 3) && !selectedSpecificIssue ? "calc(1.375rem + 1.5vw + 3rem)" : selectedSpecificIssue ? "0vh" : !selectedIssue ? "calc((100vh - 4.025rem - 0.3vw) / 3)" : "calc((100vh - 4.025rem - 0.3vw) - 2 * (1.375rem + 1.5vw + 3rem)"}}
                 onClick={() => {
                     if (selectedIssue !== 3) {
                         setSelectedIssue(3)
                     } else {
            ...
            >
                ...
                <p className={`mb-0 mobile-transition-font
                ${(selectedIssue === 3 || !selectedIssue) && !selectedSpecificIssue ? "small-text" : "no-text"}`}>
            ...
            >
                <div className={"mobile-citywide-nav"}>
                    <div className={"mobile-citywide-nav-dropdown"}
                         onClick={() => setShowDropDown(!showDropDown)}
                    >
                        <div className={"mobile-citywide-nav-text"}>
                            <div>{selectedChapter && issue_categories.labels[selectedIssue]}</div>
                            <FontAwesomeIcon icon={faChevronRight}/>
                            <div
                                className={"ellipses"}>{selectedSpecificIssue && issues.specific_issues_data[selectedSpecificIssue].specific_issue_name}</div>
                        </div>
                ...
                <div className={"position-absolute"}>
                    ...
                    >
                        <div>Health</div>
                        <FontAwesomeIcon icon={showSubDropDown === 1 ? faCaretUp : faCaretDown}/>
                    </div>

                    {issues.issues_data.health.specific_issues_ID.map((id) => {
                        return (
                            <div className={`mobile-citywide-nav-dropdown-item
                        ${showDropDown && showSubDropDown === 1 ? "mobile-citywide-nav-dropdown-item-grow" : "mobile-citywide-nav-dropdown-item-shrink"}  
                        ${selectedSpecificIssue === id ? "active-scheme" : "inactive-scheme"}
                        `}
                                 onClick={() => {
                    ...
                    <div className={`mobile-citywide-nav-dropdown-item
                        ${showDropDown ? "mobile-citywide-nav-dropdown-item-grow" : "mobile-citywide-nav-dropdown-item-shrink"}  
                        ${showSubDropDown === 2 ? "active-scheme" : "inactive-scheme"}
                        `}
                         onClick={() => {
                    ...
                    >
                        <div>Environment</div>
                        <FontAwesomeIcon icon={showSubDropDown === 2 ? faCaretUp : faCaretDown}/>
                    </div>

                    {issues.issues_data.environment.specific_issues_ID.map((id) => {
                        return (
                            <div className={`mobile-citywide-nav-dropdown-item
                        ${showDropDown && showSubDropDown === 2 ? "mobile-citywide-nav-dropdown-item-grow" : "mobile-citywide-nav-dropdown-item-shrink"}  
                        ${selectedSpecificIssue === id ? "active-scheme" : "inactive-scheme"}
                        `}
                                 onClick={() => {
                                     if (selectedSpecificIssue !== id) {
                    ...
                    <div className={`mobile-citywide-nav-dropdown-item 
                        ${showDropDown ? "mobile-citywide-nav-dropdown-item-grow" : "mobile-citywide-nav-dropdown-item-shrink"}                      
                        ${showSubDropDown === 3 ? "active-scheme" : "inactive-scheme"}
                        `}
                         onClick={() => {
                    ...
                    >
                        ...
                        <FontAwesomeIcon icon={showSubDropDown === 3 ? faCaretUp : faCaretDown}/>
                    </div>

                    {issues.issues_data.infrastructure.specific_issues_ID.map((id) => {
                        return (
                            <div className={`mobile-citywide-nav-dropdown-item
                        ${showDropDown && showSubDropDown === 3 ? "mobile-citywide-nav-dropdown-item-grow" : "mobile-citywide-nav-dropdown-item-shrink"}  
                        ${selectedSpecificIssue === id ? "active-scheme" : "inactive-scheme"}
                        `}
                                 onClick={() => {
                                     if (selectedSpecificIssue !== id) {
